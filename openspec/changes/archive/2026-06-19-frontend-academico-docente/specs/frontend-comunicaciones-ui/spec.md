...
El contrato esperado SHALL incluir, con base `/api/v1/comunicaciones`:
- `POST /api/v1/comunicaciones/preview` con `{ asunto_template: string; cuerpo_template: string; destinatarios: Array<{ email: string; variables: Record<string, string> }> }` → `200` con `{ items: Array<{ destinatario: string; asunto_render: string; cuerpo_render: string }> }` (F3.1). No encola.
- `POST /api/v1/comunicaciones` con `{ asunto_template: string; cuerpo_template: string; destinatarios: Array<{ email: string; variables: Record<string, string> }>; materia_id?: string; requiere_aprobacion?: bool }` → `201` con datos del lote creado. Encola en estado `pendiente` (RN-15).
- `GET /api/v1/comunicaciones?materia_id=&destinatario=` → `200` con `Array<ComunicacionResponse>`. Cada item incluye: `id`, `destinatario`, `asunto`, `cuerpo`, `estado` (pendiente|enviando|enviado|error|cancelado), `lote_id`, `requiere_aprobacion`.
- `GET /api/v1/comunicaciones/lote/{lote_id}` → `200` con datos del lote (total, estados por item).
- `POST /api/v1/comunicaciones/{comunicacion_id}/aprobar` y `POST /api/v1/comunicaciones/{comunicacion_id}/cancelar` → `200` con la comunicación actualizada.
- `POST /api/v1/comunicaciones/lote/{lote_id}/aprobar` y `POST /api/v1/comunicaciones/lote/{lote_id}/cancelar` → `200` con resumen de la acción.

#### Scenario: Preview muestra asunto y cuerpo por destinatario
- **WHEN** el usuario ingresa un asunto_template, cuerpo_template y selecciona destinatarios
- **THEN** se llama a `POST /api/v1/comunicaciones/preview` y la UI muestra el asunto y cuerpo renderizado por cada destinatario, sin haber encolado nada

#### Scenario: Envío encola en estado pendiente
- **WHEN** el usuario confirma el envío desde el preview
- **THEN** se llama a `POST /api/v1/comunicaciones`, los mensajes aparecen en la cola en estado `pendiente` y la UI indica si el lote `requiere_aprobacion`
...
- **THEN** se llama a `POST /api/v1/comunicaciones/lote/{lote_id}/aprobar`, se invalida la query de la cola y los mensajes transicionan hacia `enviando`/`ok`

#### Scenario: Cancelar por destinatario
- **WHEN** un aprobador cancela una comunicación individual pendiente
- **THEN** se llama a `POST /api/v1/comunicaciones/{comunicacion_id}/cancelar` y esa fila pasa a estado `cancelado`
