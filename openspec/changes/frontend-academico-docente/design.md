...
El reto particular: el backend de las features consumidas (`C-06`, `C-07`, `C-09`, `C-10`, `C-11`, `C-12`) **todavía no existe**. Por lo tanto esta capa frontend se diseña contra **contratos de API esperados** que esta propia propuesta fija, y se valida íntegramente con mocks de Axios en los tests (regla dura: tests sin tocar backend; en frontend el equivalente es mock del cliente HTTP, no de la lógica de UI).
...
- **[Contratos de backend pueden divergir de lo documentado]** → Se aísla todo el acoplamiento en `services/` + `types/`; componentes y hooks dependen de tipos de dominio estables. Re-mapear ante divergencia es local y barato. Los specs marcan los contratos como "esperados, sujetos a confirmación al implementar C-09…C-12".
